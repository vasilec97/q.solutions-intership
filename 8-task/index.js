...
function validateStartDate(startDate, instance) {
  ...
  if (startDate < today || startDate > today) {
    ...
    alert(i18next.t("alerts.date-start"))
    return false
  }
...
function validateEndDate(endDate, instance) {
  ...
  if (endDate > maxEndDate) {
    ...
    alert(i18next.t("alerts.date-end"))
    return false
  }
...
async function startProcessing() {
  ...
  const filtredContacts =
    templateOptionsMap[$templateSelect.value] === "Birthday" ? filterContacts(contacts) : contacts
...
async function retrieveAllContacts() {
  ...
  function getContactsPage(nextLink) {
    const filter =
      templateOptionsMap[$templateSelect.value] === "Birthday"
        ? `&$filter=birthdate ne null and nw_last_birthday_congrat eq null`
        : ""

    let query = nextLink
      ? nextLink
      : `?$select=birthdate,emailaddress1,firstname,lastname,gendercode${filter}`
...
async function sendCongratulations(contacts) {
  ...
  for (const [idx, contact] of contacts.entries()) {
    ...
    try {
      ...
      await GlobalHelper.sendCongratulationEmail({
        firstName: firstname,
        lastName: lastname,
        email,
        genderValue,
        language: i18next.language,
        templateTypeValue: $templateSelect.value,
      })
